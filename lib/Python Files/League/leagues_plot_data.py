...
def attack_vs_defence(league):
   ...
   try:
        ...
        for i in range(len(standings["Rk"])):
            attack_vs_defence_data[standings["Squad"][i]]={"Goals Per Game":goals_per_match[i], "Goals Conceded Per Game":goals_conceded_per_match[i]}
...
def defensive_solidity(league):
    ...
    try:
        defensive_actions = pd.read_csv(f"assets\Data\{league}\{league}_Squad_Defensive_Actions.csv")
        ...
        for i in range(len(defensive_actions["Squad"])):
            defensive_solidity_data[defensive_actions["Squad"][i]] = {"Tackles Per 90": tackles_p90[i],"Tackles Won Per 90": tackles_won_p90[i]}
        return defensive_solidity_data
    except Exception as e:
        print("Something went wrong:",e)

defensive_solidity("La Liga")
# attack_vs_defence("Bundesliga")
