...
def tis_csv_update(pos):
    ...
    if(pos=='CB'):
        ...
        df["Defensive Actions"] = (tackles+interceptions+blocks+clearances+recoveries)/nineties
        # Aerial Duels
        ...
        df["Passing and Build-up"] = (key_passes+pass_cmp+progressive_passes)/(nineties*10)
        ...
        df["Discipline"] = (yellow_cards+second_yellow_card+red_cards+fouls)/nineties
    if(pos=='LB' or pos=='RB'):
        # Defensive Duties
        ...
        clearances = np.array(df["Clr"])
        recoveries = np.array(df["Recov"])
        df["Defensive Duties"] = (tackles_Def_3rd+interceptions+blocks+clearances+recoveries)/nineties
        # Offensive Contribution
        prg_carries = np.array(df["PrgC"])
        prg_passes = np.array(df["PrgP"])
        key_passes = np.array(df["KP"])
        xA = np.array(df["xA"])
        df["Offensive Contributions"] = (prg_carries+prg_passes+xA+key_passes)/nineties
        # Final Third Play 
        ...
        df["Final Third Play"] = (crosses_attempted+shot_creating_actions+carries_penalty_area+passes_penalty_area)/nineties
        # Possession Play
    ...
    if(pos=='CDM'):
        ...
        clearances = np.array(df["Clr"])
        recoveries = np.array(df["Recov"])
        df["Defensive Contributions"] = (tackles+interceptions+blocks+clearances+recoveries)/nineties
        # Passing Ability
        pass_cmp = np.array(df["Cmp%"])
        ...
        recoveries = np.array(df["Recov"])
        interceptions = np.array(df["Int"])
        df["Ball Recovery & Defensive Work"] = (recoveries+interceptions)/(nineties*10)
        # Defensive Line Breaking Passes 
        prg_passes = np.array(df["PrgP"])
        key_passes = np.array(df["KP"])
        passes_final_third = np.array(df["Final Third"])
        df["Line Breaking Passes"] = (key_passes+prg_passes+passes_final_third)/(nineties*10)
    if(pos=="CM"):
        # Passing and Vision
        prg_passes = np.array(df["PrgP"])
        passes_final_third = np.array(df["Final Third"])
        ...
        clearances = np.array(df["Clr"])
        recoveries = np.array(df["Recov"])
        df["Defensive Work"] =  (tackles+interceptions+blocks+clearances+recoveries)/nineties
        # Chance Creation
        sca = np.array(df["SCA"])
        xG = np.array(df["xG"])
        xA = np.array(df["xA"])
        ...
        passes_final_third = np.array(df["Final Third"])
        successful_take_ons = np.array(df["Succ"])
        df["Possession Retention"] = (pass_cmp+key_passes+passes_final_third+successful_take_ons)/(nineties*10)
    if(pos=="CAM"):
    ...
    if(pos=="GK"):
        ...
        key_passes = np.array(df["KP"])
        passes_final_third = np.array(df["Final Third"])
        df["Distribution Ability"] = (passes_cmp+key_passes+passes_final_third)/nineties
    df = df.fillna(0)
...
def player_table(position, name_list):
    # Define the CSV path
    csv_path = f"D:\\College\\Final Year Project\\Front end\\player_replacement\\assets\\data\\playerdata\\{position}.csv"
    ...
    output_path = f"D:\\College\\Final Year Project\\Front end\\player_replacement\\assets\\data\\playerdata\\{position}_filtered.csv"
    result_df.to_csv(output_path, index=False)
    
    

# Example usage
player_table("CF", ['Robert Lewandowski', 'Harry Kane', 'Erling Haaland', 'Kylian Mbappé','Lautaro Martínez'])
...
def advanced_top_performers(league, top_performers_df):
    path = f"D:\\College\\Final Year Project\\Front end\\player_replacement\\assets\\data\\playerdata\\Players_Leaguewise\\{league}_players.csv"
    ...
    for p in info.findAll("p"):
        ...
        if strong and a and span:
            if strong.text.strip() == "Most Goals":  # Strip to avoid leading/trailing spaces
                top_performers_df['Top Scorer Team'] = p.text.split("(")[-1].split(")")[0]
            ...
            if strong.text.strip() == "Most Assists":  # Strip to avoid leading/trailing spaces
                top_performers_df['Top Assister Team'] = p.text.split("(")[-1].split(")")[0]
                player_name = a.text.encode("utf-8").strip()
            ...
            if strong.text.strip() == "Most Clean Sheets":  # Strip to avoid leading/trailing spaces
                top_performers_df['Most CS Team'] = p.text.split("(")[-1].split(")")[0]
                player_name = a.text.encode("utf-8").strip()
                top_performers_df['Clean Sheets Image'] = image_getter(player_name)
